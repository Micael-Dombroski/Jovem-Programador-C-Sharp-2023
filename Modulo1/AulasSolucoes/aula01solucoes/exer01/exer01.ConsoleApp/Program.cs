        ...
        static void Main(string[] args)
        {
            ...
            while (ler.ToLower() == "sim")
            {
                do {
                    ...
                    {
                        tudoCerto = false;
                        throw new ExceptionRespostaInvalida("A opção informada não é uma opção existente");
                    } 
                    switch (tipo)
                    {
                        case 1:
                            ...
                            while (analiseTamanho != true || true != soNumeros)
                            {
                                ...
                                {
                                    tudoCerto = false;
                                    throw new ExceptionMuitoPequeno("O número de caracteres inseridos não bate com a quantidade que essa informação exige");;
                                }
                                for (int i = 0; i < analiseCPF.Length; i++)
                                {
                                    ...
                                    {
                                        tudoCerto = false;
                                        throw new ExceptionApenasNumeros("Use apenas números nessa informação");
                                    } else
                                    {
                                        verificar++;
                                    }
                            ...
                            if (Index > 0)
                            {
                                for (int i = 0; i < CPFs.Length; i++)
                                {
                                    ...
                                    {
                                        tudoCerto = false;
                                        throw new ExceptionJaCadastrado("Esse valor já foi cadastrado");
                                    }
                                    if (i == CPFs.Length-1)
                                    {
                                        cpf = $"{analiseCPF[0]}{analiseCPF[1]}{analiseCPF[2]}.{analiseCPF[3]}{analiseCPF[4]}{analiseCPF[5]}.{analiseCPF[6]}{analiseCPF[7]}{analiseCPF[8]}-{analiseCPF[9]}{analiseCPF[10]}";
