...
insert into aluno (Matricula, Nome, Idade) Values (123, 'Lucas', 23)
...
insert into Materia (nome, quantidade_aulas) Values ('Math', 230)

insert into Materia_Cursada (matricula_aluno, materia_id, nota) Values (123, 1, 10)
...
/*select A.Matricula, A.Nome, A.Idade, C.id, C.nome, C.quantidade_aulas, B.nota from ALUNO A 
    ...
    INNER JOIN Materia_Cursada B ON (A.Matricula = B.matricula_aluno)    WHERE B.Id = 2;*/ 
    SELECT Id, Nome, quantidade_aulas FROM Materia WHERE Nome Like '%at%';